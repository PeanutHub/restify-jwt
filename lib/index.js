    ...
    async.parallel(
      ...
      function(err, results) {
        ...
        jwt.verify(token, secret, options, function(err, decoded) {
          ...
          const routeOpts = options.getRouteOptions(`${route.method} ${route.path}`);
          ...
          decoded.hasRole = rolesToCheck => {
            ...
            // Extract all diferent application that we need to check
            const apps = {};
            ...
            for (let i = 0; i < keys.length; i++) {
              const appId = keys[i];
              const rolesToCheck = apps[appId];
              try {
                // checkIAMSync throw error when hasn't access
                hasAtLeastOneRole = decoded.checkIAMSync(rolesToCheck, appId);
              } catch (ex) {
